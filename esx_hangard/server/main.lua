...
AddEventHandler('esx_hangard:addCarToParking', function(vehicleProps)
	...
	if vehicleProps ~= nil then
		...
		MySQL.Async.execute('INSERT INTO `user_hangard` (`identifier`, `plate`, `vehicle`) VALUES (@identifier, @plate, @vehicle)', {
...
AddEventHandler('esx_hangard:removeCarFromParking', function(plate)
	...
	if plate ~= nil then
		...
		MySQL.Async.execute('DELETE FROM `user_hangard` WHERE `identifier` = @identifier AND `plate` = @plate', {
				['@identifier'] = xPlayer.identifier,
        			['@plate'] = plate
			}, function(rowsChanged)
...
AddEventHandler('esx_hangard:getCustomPlate', function(plate)
    ...
    MySQL.Async.fetchAll('SELECT `plate_name` FROM custom_plate WHERE original_plate = @plate', {
        ['@plate'] = plate
      }, function(result)
...
ESX.RegisterServerCallback('esx_hangard:getVehiclesInGarage', function(source, cb)
	...
	MySQL.Async.fetchAll('SELECT * FROM `user_hangard` WHERE `identifier` = @identifier', {
			['@identifier'] = xPlayer.identifier
		}, function(result)
